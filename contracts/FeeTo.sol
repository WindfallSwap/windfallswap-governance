...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IWindfallPair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IWindfallPair(pair).token1()];
        ...
        if (
            ...
            value = IWindfallPair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IWindfallPair(pair).transfer(pair, value));
                IWindfallPair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IWindfallPair(pair).transfer(feeRecipient, value));
            }
        }
    }
}
