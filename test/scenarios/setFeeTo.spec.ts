...
describe('scenario:setFeeTo', () => {
  ...
  beforeEach('deploy windfall swap', async () => {
    factory = await deployContract(wallet, WindfallFactory, [timelock.address])
  })
  ...
  it('setFeeTo', async () => {
    ...
    const description = 'Set feeTo on the WindfallFactory to the timelock address.'

    // activate balances
    await wfl.delegate(wallet.address)
