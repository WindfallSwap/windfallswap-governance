...
import WindfallPair from '@windfallswap/core/build/WindfallPair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy windfall swap', async () => {
    factory = await deployContract(wallet, WindfallFactory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Wfl, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Wfl, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, WindfallPair.abi).connect(wallet)
