...
export async function governanceFixture(
  ...
  // deploy WFL, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const wfl = await deployContract(wallet, Wfl, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, wfl.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { wfl, timelock, governorAlpha }
}
