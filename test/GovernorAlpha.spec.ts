...
describe('GovernorAlpha', () => {
  ...
  it('wfl', async () => {
    const balance = await wfl.balanceOf(wallet.address)
    const totalSupply = await wfl.totalSupply()
  ...
  it('governor', async () => {
    ...
    const wflFromGovernor = await governorAlpha.wfl()
    expect(wflFromGovernor).to.be.eq(wfl.address)
  })
})
